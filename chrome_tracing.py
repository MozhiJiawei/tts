...
class ChromeTracer:
    ...
    def _emit(self, ph: str, name: str, cat: str = "", args: Optional[dict] = None,
              dur_us: Optional[int] = None, tid: Optional[int] = None) -> None:
        """发送一条 trace 事件。"""
        ts = self._ts_us()
        ev: dict[str, Any] = {
            "name": name,
    ...
    def begin(self, name: str, cat: str = "", args: Optional[dict] = None) -> None:
        """区间开始 (Phase B)。"""
        self._emit("B", name, cat=cat, args=args)

    def end(self, name: str, cat: str = "", args: Optional[dict] = None) -> None:
        """区间结束 (Phase E)。"""
        self._emit("E", name, cat=cat, args=args)

    def complete(self, name: str, dur_us: int, cat: str = "", args: Optional[dict] = None) -> None:
        """完整区间 (Phase X)，一条记录包含时长。"""
        self._emit("X", name, cat=cat, args=args, dur_us=dur_us)

    def instant(self, name: str, cat: str = "", args: Optional[dict] = None) -> None:
        """瞬时事件 (Phase i)。"""
        self._emit("i", name, cat=cat, args=args)

    def metadata(self, name: str, args: dict) -> None:
        """元数据 (Phase M)，如 process_name、thread_name。"""
        self._emit("M", name, args=args)
    ...
    def set_thread_name(self, name: str, tid: Optional[int] = None) -> None:
        ...
        self.metadata("thread_name", {"name": name})
    ...
    def span(self, name: str, cat: str = "", args: Optional[dict] = None):
        ...
        try:
            ...
            self.end(name, cat=cat, args=args)
