...
class PriorityTokenPool:
    ...
    def put(self, token_data):
        ...
        if stream_id not in self.pool:
            logger.info(f'收到首个 token: {stream_id}')
        ...
        if token_data['is_eos']:
            logger.info(f'LLM 输出结束: {stream_id}')
        ...
        # batch_queue 为空时主动通知 FM 线程，避免 token 已入池却迟迟不触发组批
    ...
    def notify_batch(self):
        ...
        with trace_span('pool.notify_batch', cat='pool', args=notify_args):
            ...
            for stream_id in self.pool.keys():
                logger.info(f'Token 池状态 stream_id: {stream_id}, queue_size: {self.pool[stream_id].qsize()}')

            # enough 数量还没到最大 batch 时，优先等待接近阈值的流一起组批
            if len(token_stat['enough']) < self.max_batch_size and len(token_stat['close']) > 0:
                logger.info(f'接近 token 阈值，继续等待 stream_ids: {token_stat["close"]}')
                trace_instant(
